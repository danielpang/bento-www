...
- Both variants carry the same product FAQ (with FAQPage markup) after the integrations section. It was added to the control after it was preserved so that search and answer engines see it whichever variant is served; it is identical in both, so the comparison stays about the treatment.
